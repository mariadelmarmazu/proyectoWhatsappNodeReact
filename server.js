...
app.post('/whatsapp', async (req, res) => {
  ...
  const insertOrGetSubfamiliaId = (id_familia, subfamilia, callback) => {
    const query = 'SELECT id_subfamilia FROM subfamilia WHERE subfamilia = ? AND id_familia = ?';
    db.query(query, [subfamilia, id_familia], (err, results) => {
      if (err) {
        return callback(err);
      }
      if (results.length > 0) {
      ...
      const insertQuery = 'INSERT INTO subfamilia (subfamilia, id_familia) VALUES (?, ?)';
      db.query(insertQuery, [subfamilia, id_familia], (err, result) => {
        if (err) {
          return callback(err);
        }
        callback(null, result.insertId);
  ...
  // Insertar artículo
  const insertArticulo = (id_familia, id_subfamilia, nombre, marca, precio, imagePath, callback) => {
    const query = 'INSERT INTO articulos (id_familia, id_subfamilia, nombre, marca, precio, imagen) VALUES (?, ?, ?, ?, ?, ?)';
    db.query(query, [id_familia, id_subfamilia, nombre, marca, precio, imagePath], (err, result) => {
      if (err) {
        return callback(err);
      }
      callback(null, result.insertId);
  ...
  insertOrGetFamiliaId(familia, (err, id_familia) => {
    ...
    insertOrGetSubfamiliaId(id_familia, subfamilia, (err, id_subfamilia) => {
      if (err) {
        console.error('Error al insertar o recuperar subfamilia:', err);
        return res.sendStatus(500);
      }
      insertArticulo(id_familia, id_subfamilia, nombre, marca, precio, imageFilePath, (err, id_articulo) => {
        if (err) {
